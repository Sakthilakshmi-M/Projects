...
?>
<?php
  $name = $rpic= $phone = $address = $district = $state = $pincode = '';
  $errors = array('name'=>'','rpic' => '','phone' => '','address' => '','district' => '','state'=>'','pincode'=>'');
  ...
  if(isset($_POST['receiver']))
  {
    ...
    {
      $errors['rpic'] = 'Image is required!';
    }
    else if(getimagesize($_FILES['rpic']['tmp_name'])==false) {
      $errors['rpic'] = "Image is required";
    }
    else {
      $rpic = $_FILES['rpic']['tmp_name'];
      $name = $_FILES['rpic']['name'];

      $rpic = file_get_contents($rpic);
      $rpic = base64_encode($rpic); 
    }
  ...
  if (array_filter($errors))
  {
    ...
    $insert = "INSERT INTO receiver(name,picture,phone,address,district,state,pincode) VALUES('$name','$rpic','$phone','$address','$district','$state','$pincode')";
...
<html>
  ...
  <form action="receiverDetails.php" method="POST" enctype="multipart/form-data">
  ...
  <div class="input-group">
      ...
      <div class="input-group file">
        <label for="rpic">Location picture:</label>
        <input type = "file" name="rpic" id="rpic">
        <div class="error"><?php echo $errors['rpic'] ?></div>
      </div>
      <div class="input-group">
      ...
      <input type="submit" name="receiver" value="Update" class="btn">
  </div>
  </div>
  </div>
  </form>